...
public class RobotContainer {
  ...
  private Command generateRamseteCommand() {
    ...
    String trajectoryJSON = "paths/output/GetBall.wpilib.json";

    try {
      Path trajectoryPath = Filesystem.getDeployDirectory().toPath().resolve(trajectoryJSON);
      Trajectory pathWeaverTrajectory = TrajectoryUtil.fromPathweaverJson(trajectoryPath);
    
      RamseteCommand ramseteCommand = new RamseteCommand(pathWeaverTrajectory, m_drivetrain::getPose,
      ...
      m_drivetrain.putTrajectory(pathWeaverTrajectory);
      // Set up a sequence of commands
      // First, we want to reset the drivetrain odometry
      return new InstantCommand(() -> m_drivetrain.resetOdometry(pathWeaverTrajectory.getInitialPose()), m_drivetrain)
  ...
  private void configureButtonBindings() {
    ...
    onboardButtonA.whenActive(new PrintCommand("Button A Pressed")).whenInactive(new PrintCommand("Button A Released"))
        .toggleWhenActive(generateRamseteCommand());
